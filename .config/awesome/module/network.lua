...
local apps = require('configuration')
...
local function color(icon_raw)
	if icon_raw ~= nill then
		return gears.color.recolor_image(icon_raw, beautiful.xforeground)
	else 
		return nil
	end
end
...
local return_button = function()
	...
	local widget = wibox.widget {
		{
			id = 'icon',
			image = gears.color.recolor_image(widget_icon_dir .. 'wifi-strength-off' .. '.svg',beautiful.xforeground),
		...
		local update_wireless_icon = function(strength)
			awful.spawn.easy_async_with_shell(
				...
				function(stdout)
					...
					widget.icon:set_image(color(widget_icon_dir .. widget_icon_name .. '.svg'))
				end
			)
		end
